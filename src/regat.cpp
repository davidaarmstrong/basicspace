...
// The REGAT function using RcppEigen
// [[Rcpp::export]]
void REGAT(int NRESPONDENTS, int NDIMENSIONS, int NS, int NF,
           const Eigen::MatrixXd& A, const Eigen::VectorXd& Y, Eigen::VectorXd& V)
{
  ...
  // Perform eigenvalue decomposition of ZMAT
  ...
  // Compute inverse of B using eigenvalues and eigenvectors
  Eigen::MatrixXd D_inv = Eigen::MatrixXd::Zero(NF, NF);
  for (int i = 0; i < NF; ++i) {
    if (std::abs(WVEC(i)) > 0.00001) {
